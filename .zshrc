...
# umask 022 = 666 - 022 = 644（permission)
umask 022
# アプリケーションが異常終了するとダンプされた情報が
#「core」のファイル名で書き出される。このcoreファイルは，
# 解析しない限り意味のないファイル
## limit coredumpsize 数値 でサイズ指定できる
...
#autoload -Uz zcalc
autoload -Uz zmv  # zmv -W '*.lis' '*.txt'でワイルドカードでリネーム
autoload     run-help-git
autoload     run-help-svk
autoload     run-help-svn

# It is necessary for the setting of DOTPATH
[ -f ~/.path ] && source ~/.path # NOTE : .pathをシンボリック化しよう! (.path -> ~/dotfiles/.path)に 
...
setup_bundles() {
    ...
    bundles() {
	    ...
	    # Then, source plugins and add command to $PATH
            zplug load --verbose && e_done "Ready"
        else
            echo "$fg[red]To make your shell strong, run 'bundle_install'.$reset_color"
            printf "exec 'bundle_install'? [y/N]: "
            if read -q; then
              echo "installing..." | e_indent 2
              echo; bundle_install
            fi
        fi
    }
...
zshrc_keybind() {
    ...
    bindkey -M viins '^]' insert-last-word  # NOTE : Ctrl + ]で挿入
    ...
    bindkey -M viins '^Q' quote-previous-word-in-single  # NOTE : Ctrl + Qで前の単語をシングルクオートで囲む
    ...
    bindkey -M viins '^Xq' quote-previous-word-in-double  # NOTE : Ctrl + Xqでダブルクオートで囲む
    bindkey -M viins "$terminfo[kcbt]" reverse-menu-complete
}

zshrc_setopt()
{
    setopt auto_cd  # cdと打たなくても自動で移動してくれる
    setopt auto_pushd  # TODO : ディレクトリスタックの操作を簡単に行うもの、いくつかの組み合わせで、楽にディレクトリ移動できる

    # Do not print the directory stack after pushd or popd.
    #setopt pushd_silent
    # Replace 'cd -' with 'cd +'
    setopt pushd_minus  #移動履歴をキューではなくスタックで番号付け(最近居た場所が上になる)

    # Ignore duplicates to add to pushd
    setopt pushd_ignore_dups  #移動履歴の重複禁止
